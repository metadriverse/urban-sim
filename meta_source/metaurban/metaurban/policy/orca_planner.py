...
sys.path.insert(0, "meta_source/metaurban/metaurban/orca_algo/build")
...
class OrcaPlanner:
    ...
    def get_planning(self, start_positions, goals, num_agent, walkable_regions_mask=None):
        def get_speed(positions):
            ...
            return list(speed)
        self.set_agents(start_positions, goals, self.template_xml_file) ## overwrite template_xml_file by new agent's starts & goal position
    ...
    def get_next(self, return_speed=False):
        if not return_speed:
            ...
            if not self.has_next(): return None, None
