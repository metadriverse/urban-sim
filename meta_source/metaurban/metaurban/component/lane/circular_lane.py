...
from metaurban.utils.math import wrap_to_pi, norm, Vector
...
class CircularLane(PGLane):
    ...
    def __init__(
        ...
        priority: int = 0,
        metaurban_type=MetaUrbanType.LANE_SURFACE_STREET,
    ) -> None:
        assert angle > 0, "Angle should be greater than 0"
        super().__init__(metaurban_type)
    ...
    def position(self, longitudinal: float, lateral: float, delta: float=0) -> Vector:
        phi = self.direction * longitudinal / (self.radius + delta) + self.start_phase
        ...
        return self.center + ((self.radius + delta) + lateral * self.direction) * Vector((math.cos(phi), math.sin(phi)))
