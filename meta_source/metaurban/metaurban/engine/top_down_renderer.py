...
from metaurban.component.map.scenario_map import ScenarioMap
from metaurban.constants import Decoration, TARGET_VEHICLES
from metaurban.constants import TopDownSemanticColor, MetaUrbanType, PGDrivableAreaProperty
from metaurban.obs.top_down_obs_impl import WorldSurface, ObjectGraphics, LaneGraphics, history_object
from metaurban.scenario.scenario_description import ScenarioDescription
from metaurban.utils.utils import import_pygame
from metaurban.utils.utils import is_map_related_instance
...
def draw_top_down_map_native(
    ...
    Args:
        map: MetaUrban.BaseMap instance
    ...
    if semantic_map:
        ...
        for obj in all_lanes.values():
            if MetaUrbanType.is_lane(obj["type"]):
                pygame.draw.polygon(
                    surface, TopDownSemanticColor.get_color(obj["type"]),
                    [surface.pos2pix(p[0], p[1]) for p in obj["polygon"]]
                )

            elif MetaUrbanType.is_road_line(obj["type"]) or MetaUrbanType.is_road_boundary_line(obj["type"]):
                if semantic_broken_line and MetaUrbanType.is_broken_line(obj["type"]):
    ...
    def _append_frame_objects(objects):
        ...
        for name, obj in objects.items():
            frame_objects.append(
                history_object(
                    name=name,
                    type=obj.metaurban_type if hasattr(obj, "metaurban_type") else MetaUrbanType.OTHER,
