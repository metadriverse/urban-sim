...
class SidewalkDynamicMetaUrbanEnv(BaseEnv):
    ...
    def reward_function(self, vehicle_id: str):
        ...
        lateral_penalty = np.exp(-lateral_factor * self.config["lateral_penalty"])
        ...
        heading_penalty = np.exp(-heading_diff * self.config["heading_penalty"])
        ...
        if vehicle_id not in self.previous_agent_actions or "steering_penalty" not in self.config or self.config["steering_penalty"] == 0:
            ...
            steering_reward = np.exp(-steering_diff * self.config["steering_penalty"])
