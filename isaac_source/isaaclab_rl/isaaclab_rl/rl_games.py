...
class RlGamesVecEnvWrapper(IVecEnv):
    ...
    @property
    def sensor_observation_space(self):
        """Returns the :attr:`Env` :attr:`observation_space`."""
        # note: rl-games only wants single observation space
        try:
            sensor_obs_space = self.unwrapped.single_observation_space["sensor"]
        except:
            return None
        if not isinstance(sensor_obs_space, gymnasium.spaces.Box):
            raise NotImplementedError(
                f"The RL-Games wrapper does not currently support observation space: '{type(sensor_obs_space)}'."
                f" If you need to support this, please modify the wrapper: {self.__class__.__name__},"
                " and if you are nice, please send a merge-request."
            )
        # note: maybe should check if we are a sub-set of the actual space. don't do it right now since
        #   in ManagerBasedRLEnv we are setting action space as (-inf, inf).
        return gym.spaces.Box(-self._clip_obs, self._clip_obs, sensor_obs_space.shape)
    ...
    def get_env_info(self) -> dict:
        ...
        return {
            ...
            'sensor_observation_space': self.sensor_observation_space,
    ...
    def _process_obs(self, obs_dict: VecEnvObs) -> torch.Tensor | dict[str, torch.Tensor]:
        ...
        """
        # process policy obs
        if 'sensor' in obs_dict:
            sensor_obs = obs_dict["sensor"]
            sensor_obs = sensor_obs.to(device=self._rl_device).clone()
            ...
            if self.rlg_num_states > 0:
                ...
                return {"obs":  {"obs_addons": obs, 'obs_sensor': sensor_obs}, "states": states}
            else:
                return {"obs_addons": obs, 'obs_sensor': sensor_obs}
