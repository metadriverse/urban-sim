...
class RigidObject(AssetBase):
    ...
    def __init__(self, cfg: RigidObjectCfg):
        ...
        elif asset_path_is_regex:
            self.all_assets = self.cfg.spawn.func(
                self.cfg.prim_path,
                self.cfg.spawn,
            )
    ...
    def body_names(self) -> list[str]:
        ...
        if hasattr(self, 'all_assets'):
            return [path[0].split("/")[-1] for path in self.all_assets]
    ...
    def _initialize_impl(self):
        ...
        if hasattr(self, 'all_assets'):
            self._ALL_BODY_INDICES = torch.arange(
                len(self.all_assets), dtype=torch.long, device=self.device
            )
            # external forces and torques
            self.has_external_wrench = False
            self._external_force_b = torch.zeros((len(self.all_assets), 3), device=self.device)
