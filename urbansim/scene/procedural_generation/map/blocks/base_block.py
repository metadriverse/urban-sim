...
class BaseBlock:
    ...
    def construct_from_config(self, config: Dict, root_render_np, physics_world):
        ...
        if self.engine is None or (self.engine.global_config["show_sidewalk"]):
            for sidewalk_id, sidewalk in self.sidewalks.items():
                ...
                for polygon in polygons[int(len(polygons) // 2):]:
                    ...
                    body_node.setIntoCollideMask(CollisionGroup.Sidewalk)
                    self._node_path_list.append(np)
            if self.engine.global_config['test_slope_system']:
        ...
        if self.engine is None or (self.engine.global_config["show_sidewalk"]):
            for _, nearroad_sidewalk in self.sidewalks_near_road_buffer.items():
                ...
                for polygon in polygons:
                    ...
                    self._node_path_list.append(np)
            if self.engine.global_config['test_slope_system']:
                for _, valid_region in enumerate(self.slo_nb):
                    polygons = TerrainProperty.clip_polygon(valid_region)
                    for idx, polygon in enumerate(polygons):
        ...
        if self.engine is None or (self.engine.global_config["show_sidewalk"]):
            for _, farfromroad_sidewalk in self.sidewalks_farfrom_road_buffer.items():
                ...
                for polygon in polygons[int(len(polygons) // 2):]:
                    ...
                    self._node_path_list.append(np)
            if self.engine.global_config['test_slope_system']:
                for _, valid_region in enumerate(self.slo_fb):
                    polygons = TerrainProperty.clip_polygon(valid_region)
                    for idx, polygon in enumerate(polygons):
    ...
    def _construct_valid_region(self):
        ...
        """
        if self.engine is None or (self.engine.global_config["show_sidewalk"]):
            valid_region_num = len(list(self.valid_region.keys()))
            cur_i = -1
    ...
    def _construct_lane_line_segment(self, start_point, end_point, line_color: Vec4, line_type: PGLineType):
        ...
        if not isinstance(start_point, npy.ndarray):
            start_point = npy.array(start_point)
        if not isinstance(end_point, npy.ndarray):
            end_point = npy.array(end_point)
