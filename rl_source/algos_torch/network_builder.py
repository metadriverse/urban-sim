    ...
    class Network(NetworkBuilder.BaseNetwork):
        def _calc_input_size(self, input_shape,cnn_layers=None, extra_shape=None):
            if cnn_layers is None:
                ...
                return nn.Sequential(*cnn_layers)(torch.rand(1, *(input_shape))).flatten(1).data.size(1) + extra_shape[0] if extra_shape is not None else nn.Sequential(*cnn_layers)(torch.rand(1, *(input_shape))).flatten(1).data.size(1)
            
        def __init__(self, params, **kwargs):
            self.actions_num = actions_num = kwargs.pop('actions_num')
            input_shape = kwargs.pop('input_shape')
            #extra_shape = kwargs.pop('extra_shape')
            ...
            self.pre_mlp = self._build_mlp(**pre_mlp)
            mlp_input_shape = self._calc_input_size(cnn_input_shape, self.cnn, extra_shape=(128, ))
        ...
        def forward(self, obs_dict):
            if self.require_rewards or self.require_last_actions:
                obs_policy = obs_dict['obs']['observation']
                ...
                obs_policy = obs_dict['obs']['obs_addons']
                sensor_obs = obs_dict['obs']['obs_sensor']
            if self.permute_input:
