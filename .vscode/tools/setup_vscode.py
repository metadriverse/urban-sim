...
URBANSIM_DIR = pathlib.Path(__file__).parents[2]
...
try:
    ...
    isaacsim_dir = os.path.join(URBANSIM_DIR, "_isaac_sim")
except EOFError:
...
if not os.path.exists(isaacsim_dir):
    raise FileNotFoundError(
        ...
        f"\n\t1. The Isaac Sim directory does not exist as a symlink at: {os.path.join(URBANSIM_DIR, '_isaac_sim')}"
...
def overwrite_python_analysis_extra_paths(isaaclab_settings: str) -> str:
    ...
    if os.path.exists(isaacsim_vscode_filename):
        ...
        rel_path = os.path.relpath(ISAACSIM_DIR, URBANSIM_DIR)
    ...
    isaaclab_extensions = os.listdir(os.path.join(URBANSIM_DIR, "isaac_source"))
    path_names.extend(['"${workspaceFolder}/isaac_source/' + ext + '"' for ext in isaaclab_extensions])
...
def main():
    ...
    isaaclab_vscode_template_filename = os.path.join(URBANSIM_DIR, ".vscode", "tools", "settings.template.json")
    ...
    isaaclab_vscode_filename = os.path.join(URBANSIM_DIR, ".vscode", "settings.json")
    with open(isaaclab_vscode_filename, "w") as f:
        f.write(isaaclab_settings)

    # copy the launch.json file if it doesn't exist
    isaaclab_vscode_launch_filename = os.path.join(URBANSIM_DIR, ".vscode", "launch.json")
    isaaclab_vscode_template_launch_filename = os.path.join(URBANSIM_DIR, ".vscode", "tools", "launch.template.json")
